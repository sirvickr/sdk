...
namespace Resto.Front.Api.CustomerScreen
{
    ...
    public sealed class CustomerScreenPlugin : IFrontPlugin
    {
        ...
        public CustomerScreenPlugin()
        {
            ...
            {
                PluginContext.Log.Info("Can't show customer screen. There is no second monitor.");
                return;
            }
            ...
            try
            {
                ...
                Application.Current.Dispatcher.Invoke(new Action(() =>
                {
                    ...
                    unsubscribe.Add(PluginContext.Notifications.ScreenChanged.ObserveOn(DispatcherScheduler.Current).Subscribe(OnScreenChanged));
                    unsubscribe.Add(PluginContext.Notifications.ChangeSumChanged.ObserveOn(DispatcherScheduler.Current).Subscribe(OnChangeSumChanged));
                    unsubscribe.Add(PluginContext.Notifications.OrderChanged.ObserveOn(DispatcherScheduler.Current).Where(o => Equals(vmOrder.OrderSource, o)).Subscribe(vmOrder.Update));
                    unsubscribe.Add(PluginContext.Notifications.RestaurantChanged.ObserveOn(DispatcherScheduler.Current).Subscribe(r => OnRestaurantChanged(r.Currency)));
