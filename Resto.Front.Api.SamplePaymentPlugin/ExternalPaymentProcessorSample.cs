...
namespace Resto.Front.Api.SamplePaymentPlugin
{
    internal sealed class ExternalPaymentProcessorSample : MarshalByRefObject, IExternalPaymentProcessor, IDisposable
    {
        ...
        {
            get { return paymentSystemKey; }
        }

        public string PaymentSystemName
        {
            get { return paymentSystemName; }
        }
            ...
            IViewManager viewManager, IPaymentDataContext context, IProgressBar progressBar)
        {
            ...
            var input = viewManager.ShowInputPopup("Enter room number or slide card", InputTypes.Card | InputTypes.Number, defaultRoomNumber);
            ...
            progressBar.ChangeMessage("Long action. Information for user");
            Thread.Sleep(5000);
        ...
        public void Pay(decimal sum, Guid? orderId, Guid paymentTypeId, Guid transactionId, [NotNull] IPointOfSale pointOfSale, [NotNull] IUser cashier, IReceiptPrinter printer,
            IViewManager viewManager, IPaymentDataContext context, IProgressBar progressBar)
        {
            ...
            var slip = new ReceiptSlip
            {
                Doc =
                    new XElement(Tags.Doc,
                        new XElement(Tags.Pair, new XAttribute(Tags.Left, "Payment System"), new XAttribute(Tags.Right, PaymentSystemKey)),
                        new XElement(Tags.Pair, new XAttribute(Tags.Left, "Transaction ID"), new XAttribute(Tags.Right, transactionId.ToString())),
                        new XElement(Tags.Pair, new XAttribute(Tags.Left, "Data"), new XAttribute(Tags.Right, data.Data)),
                        new XElement(Tags.Pair, new XAttribute(Tags.Left, "was card"), new XAttribute(Tags.Right, data.IsCard ? "YES" : "NO")),
                        new XElement(Tags.Pair, new XAttribute(Tags.Left, "Order #"), new XAttribute(Tags.Right, order == null ? "unknown" : order.Number.ToString())),
                        new XElement(Tags.Pair, new XAttribute(Tags.Left, "Full sum"), new XAttribute(Tags.Right, order == null ? "unknown" : order.FullSum.ToString())),
                        new XElement(Tags.Pair, new XAttribute(Tags.Left, "Sum to pay"), new XAttribute(Tags.Right, order == null ? "unknown" : order.ResultSum.ToString())),
                        new XElement(Tags.Pair, new XAttribute(Tags.Left, "Sum to process"), new XAttribute(Tags.Right, sum.ToString())))
            };
            printer.Print(slip);
            ...
            IViewManager viewManager, IPaymentDataContext context, IProgressBar progressBar)
        {
            ...
            var slip = new ReceiptSlip
            {
                Doc = new XElement(Tags.Doc,
                    new XElement(Tags.Pair, new XElement(Tags.Left, "Return"), new XAttribute(Tags.Right, PaymentSystemKey)),
                    new XElement(Tags.Pair, new XElement(Tags.Left, "Transaction ID"), new XAttribute(Tags.Right, transactionId.ToString())),
                    new XElement(Tags.Pair, new XElement(Tags.Left, "was card"), new XAttribute(Tags.Right, data.IsCard ? "YES" : "NO")))
            };
            printer.Print(slip);
