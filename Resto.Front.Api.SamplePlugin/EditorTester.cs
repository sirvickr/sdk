...
using Resto.Front.Api.V6.Data.Brd;
using Resto.Front.Api.V6.Data.DataTransferObjects;
...
using Resto.Front.Api.V6.Editors.Stubs;
...
namespace Resto.Front.Api.SamplePlugin
{
    internal sealed class EditorTester : IDisposable
    {
        ...
        private void EntryPoint()
        {
            ...
            AddButton("Get tables", GetTables);
            AddButton("Get orders", GetOrders);

            AddButton("Create order", CreateOrder);
            AddButton("Add guest", AddGuest);
            AddButton("Add product (Start Host)", AddProduct);
        ...
        {
            string msg = "Tables:";
            //PluginContext.Operations.AddNotificationMessage("GetTables", "Editor tester", TimeSpan.FromSeconds(5));
            IReadOnlyList<ITable> tables = PluginContext.Operations.GetTables();
            //tables.ForEach<ITable>
            foreach (ITable table in tables)
            {
                //PluginContext.Log.Info(table.Id.ToString());
                msg += " {" + table.Id.ToString() + " " + table.Number.ToString() + "}\n";
            }
            PluginContext.Operations.AddNotificationMessage(msg, "Editor tester", TimeSpan.FromSeconds(5));
        }
        ...
        private void GetOrders()
        {
            string msg = "Orders:\n";
#if false
            //PluginContext.Operations.AddNotificationMessage("GetOrders", "Editor tester", TimeSpan.FromSeconds(5));
            IReadOnlyList<IOrder> orders = PluginContext.Operations.GetOrders();
            foreach (IOrder order in orders)
            {
                msg += " {";
                Order item = new Order { Id = order.Id/*, TableId = order.Tables.Count*/ };
                msg += " " + order.Id.ToString() + " SUM " + order.FullSum;
                IReadOnlyList<ITable> tables = order.Tables;
                msg += " [";
                foreach (ITable table in tables)
                {
                    msg += " " + table.Number.ToString();
                    //item.Tables.Add(new Table { Id = Guid.NewGuid(), Number = 10 });
                }
                msg += " ]";
                msg += " }\n";
            }
